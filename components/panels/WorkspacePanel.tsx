...
// ── OPERATIONS TAB ─────────────────────────────────────────────────────────────
function OperationsTab({ suppliers, skus }: { suppliers: Supplier[]; skus: SKU[] }) {
  ...
  const openEdit = (p: Project) => {
    ...
    setForm({ name:p.name, description:p.description, status:p.status, clientName:p.clientName ?? "",
      supplierId:p.supplierId ?? "", warehouseLocation:p.warehouseLocation ?? "",
      targetAddress:p.targetAddress ?? "", paymentTerms:p.paymentTerms ?? "",
  ...
  const visibleList = (tab === "active" ? active : past).filter(p => !search || p.name.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 shrink-0">
        ...
        <div className="flex-1 text-xs font-medium text-slate-600">{view==="edit" ? (isNew ? "New Operation" : form.name) : "Operations"}</div>
        {view === "list" && (
          <div className="flex items-center gap-2">
            <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
              {(["active","history"] as const).map(t => (
                <button key={t} onClick={() => setTab(t)} className={`px-2.5 py-1 rounded-md text-xs font-semibold capitalize transition-colors ${tab===t?"bg-white text-slate-800 shadow-sm":"text-slate-500 hover:text-slate-700"}`}>
                  {t === "active" ? "Active" : "History"}
                </button>
              ))}
            </div>
            {tab === "active" && <button onClick={openNew} className="text-xs bg-blue-600 text-white font-semibold px-2.5 py-1 rounded-lg hover:bg-blue-700">+ New</button>}
          </div>
        )}
      </div>
      ...
      {view === "list" ? (
        <>
          <div className="px-4 pt-2 pb-2 shrink-0">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {visibleList.length === 0 && <p className="px-4 py-8 text-center text-xs text-slate-400">{tab==="active" ? (active.length===0 ? "No active operations" : "No matches") : "No past operations"}</p>}
            {visibleList.map(p => {
              ...
              return (
                <button key={p.id} onClick={() => openEdit(p)} className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium capitalize shrink-0 ${PROJ_STATUS_CLS[p.status]}`}>{p.status.replace("_"," ")}</span>
                  </div>
                  {p.clientName && <p className="text-xs text-slate-500 truncate mb-0.5">Client: {p.clientName}</p>}
                  <div className="flex items-center gap-3">
                    {p.endDate && <span className="text-xs text-slate-400">{p.endDate}</span>}
                    {pct != null && (
                      <div className="flex items-center gap-1.5 flex-1">
                        <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-purple-400 rounded-full" style={{width:`${pct}%`}} /></div>
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400 pt-1 pb-1 border-b border-slate-100">Client & Logistics</p>
            ...
            <F label="Warehouse Location"><input value={form.warehouseLocation} onChange={e => f("warehouseLocation",e.target.value)} placeholder="e.g. Warehouse A, Manchester" className={inp} /></F>
            <F label="Target Address"><input value={form.targetAddress} onChange={e => f("targetAddress",e.target.value)} placeholder="Delivery address" className={inp} /></F>
            <F label="Payment Terms"><input value={form.paymentTerms} onChange={e => f("paymentTerms",e.target.value)} placeholder="Net 30, COD, etc." className={inp} /></F>
            ...
            {!isNew && selected && (
              <div>
                <div className="flex items-center justify-between mb-2 pt-1">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">SKU Allocation</p>
                  <button onClick={() => setAddingItem(v=>!v)} className="text-xs text-blue-600 hover:text-blue-800 font-medium">+ Add</button>
                </div>
                {/* Suggested allocation notice */}
                {selected.items.length > 0 && (() => {
                  const underAllocated = selected.items.filter(i => i.qtyAllocated < i.qtyRequired);
                  return underAllocated.length > 0 ? (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-2">
                      <p className="text-xs font-semibold text-amber-700 mb-0.5">Resource Allocation Suggested</p>
                      {underAllocated.map(i => (
                        <p key={i.skuId} className="text-[10px] text-amber-600">{skuLabel(i.skuId)}: needs {i.qtyRequired - i.qtyAllocated} more units</p>
                      ))}
                    </div>
                  ) : null;
                })()}
                  ...
                  {selected.items.map((item, i) => {
                    ...
                    return (
                      <div key={i} className="bg-slate-50 rounded px-3 py-2">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1 bg-slate-200 rounded-full"><div className="h-full bg-purple-400 rounded-full" style={{width:`${pct}%`}} /></div>
                          <span className="text-[10px] text-slate-400">{item.qtyAllocated}/{item.qtyRequired} ({pct}%)</span>
                        </div>
                      </div>
                    );
                  })}
          ...
          <div className="shrink-0 border-t border-slate-100 px-4 py-3 flex gap-3 bg-white">
            <button onClick={save} disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold text-sm py-2.5 rounded-lg">{saving ? "Saving…" : isNew ? "Create Operation" : "Save Changes"}</button>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 shrink-0">
        <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
          {([false,true]).map(h => (
            <button key={String(h)} onClick={() => setHistoryTab(h)} className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-colors ${historyTab===h?"bg-white text-slate-800 shadow-sm":"text-slate-500 hover:text-slate-700"}`}>
              {h ? "History" : "Stock"}
            </button>
          ))}
        </div>
        {!historyTab && <button onClick={() => setShowAdd(v=>!v)} className="text-xs bg-blue-600 text-white font-semibold px-2.5 py-1 rounded-lg hover:bg-blue-700">+ Add</button>}
      </div>
      ...
      {!historyTab && (
        <div className="flex gap-2 px-4 pt-2 pb-2 shrink-0">
          ...
          <select value={filterLoc} onChange={e=>setFilterLoc(e.target.value)} className="border border-slate-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 max-w-[130px]">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
        {sorted.length === 0 && <p className="px-4 py-8 text-center text-xs text-slate-400">{stock.length===0?"No stock entries yet":"No matches"}</p>}
      ...
      <div className="px-4 py-2 border-t border-slate-100 text-xs text-slate-400 shrink-0">{stock.length} entries · £{totalValue.toLocaleString(undefined,{maximumFractionDigits:0})} value</div>
    </div>
  );
}
...
export default function WorkspacePanel({ onClose, onSelectJob, onNewJob }: Props) {
  ...
  useEffect(() => {
    ...
    ]).then(([s, sup]) => { setSkus(s); setSuppliers(sup); });
  }, []);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 shrink-0 bg-white">
        ...
        <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
          {([["operations","Ops"],["inventory","Stock"],["transits","Transits"]] as const).map(([t,lbl]) => (
            <button key={t} onClick={() => setWTab(t)} className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-colors ${wTab===t?"bg-white text-slate-800 shadow-sm":"text-slate-500 hover:text-slate-700"}`}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-hidden">
        {wTab === "operations" && <OperationsTab suppliers={suppliers} skus={skus} />}
